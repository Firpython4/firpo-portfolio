import {
  promiseFullfilledPredicate,
  promiseRejectedPredicate,
  valueMapper,
} from "~/promises/promisePredicates";

export function mapMap<KeyType, ValueType, NewKeyType, NewValueType>(
  map: Map<KeyType, ValueType>,
  mapper: (key: KeyType, value: ValueType) => [NewKeyType, NewValueType],
) {
...
export async function mapMapAsync<KeyType, ValueType, NewKeyType, NewValueType>(
  map: Map<KeyType, ValueType>,
  mapper: (
    key: KeyType,
    value: ValueType,
  ) => Promise<[NewKeyType, NewValueType]>,
) {
  const newMap = new Map<NewKeyType, NewValueType>();
